	   ...
	   if (*(Key + 1) == 'D')
		 if (*(Key + 3) == '%')
		   switch (*(Key + 2))
		   {
				case '1' :                                 // "AD8%"?
				{
					sprintf(NewKey, "%3d", t_web);     // insert pseudo-ADconverter value
					memcpy(Key, NewKey, 4);
					break;
				}
				case '2' :                                 // "AD7%"?
				{
					sprintf(NewKey, "%3d", x_web);     // copy saved value from previous read
					memcpy(Key, NewKey, 4);
					break;
				}
				case '3' :                                 // "AD1%"?
				 {
				 sprintf(NewKey, "%3d", y_web);    // increment and insert page counter
				   memcpy(Key, NewKey, 4);
				   break;
				 }
				case '4' :                                 // "AD1%"?
				 {
				 sprintf(NewKey, "%3d", z_web);    // increment and insert page counter
				   memcpy(Key, NewKey, 4);
				   break;
				 }
				...
				{
					// width da tabela
					// 15 * 7 -> nivel maximo vezes 7 para o maximo ser 100
					// multiplica por 2 pq a largura é 200 ao inves de 100
					sprintf(NewKey, "%3u", level*20);    // increment and insert page counter
					memcpy(Key, NewKey, 4);
					break;
				}
		   }
